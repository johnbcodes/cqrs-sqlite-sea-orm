...
echo -e "\n\nObtaining checks for account (calling a query)"
curl -i --location $TEST_URL/checks
echo -e "\n\nObtaining ledger entries for account (calling a query)"
curl -i --location $TEST_URL/ledger_entries
echo
